...
async function getVoiceFile(id: keyof typeof VOICES) {
  // Only try to use fs if running in Node.js (not in browser)
  if (typeof window === "undefined") {
    try {
      ...
      // Если fs не определён или нет readFile, fallback на fetch ниже
      console.warn("fs.readFile unavailable, falling back to fetch", e);
    }
  }
