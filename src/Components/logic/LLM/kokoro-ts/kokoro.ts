import { env as hf, StyleTextToSpeech2Model, AutoTokenizer, Tensor, RawAudio, ProgressCallback, PreTrainedTokenizer } from "@huggingface/transformers";
import { phonemize } from "./phonemizer/phonemizer";
...
export class KokoroTTS {
   ...
   */
  constructor(private model: StyleTextToSpeech2Model, private tokenizer: PreTrainedTokenizer) {
  }
   ...
   */
  static async from_pretrained(model_id: string, { dtype = "fp32", device = null, progress_callback = null }: { dtype?: "fp32" | "fp16" | "q8" | "q4" | "q4f16", device?: "wasm" | "webgpu" | "cpu" | null, progress_callback?: ProgressCallback } = {}) {
   ...
   */
  async generate(text: string, { voice = "af_heart", speed = 1 }: { voice?: keyof typeof VOICES, speed?: number } = {}) {
   ...
   * @returns {Promise<RawAudio>} The generated audio
   */
  async generate_from_ids(input_ids: Tensor, { voice = "af_heart", speed = 1 }: { voice?: keyof typeof VOICES, speed?: number } = {}) {
   ...
   */
  async *stream(text: string | TextSplitterStream, { voice = "af_heart", speed = 1, split_pattern = null }: { voice?: keyof typeof VOICES, speed?: number, split_pattern?: RegExp | null } = {}) {
    const language = this._validate_voice(voice);
