 ...
 * @returns {boolean}
 */
function isSentenceTerminator(c: string, includeNewlines = true) {
 ...
 * @returns {boolean}
 */
function isTrailingChar(c: string) {
  return "\"')]}」』".includes(c);
}
 ...
 */
function getTokenFromBuffer(buffer: string, start: number) {
  let end = start;
 ...
 */
function updateStack(c: string, stack: string[], i: number, buffer: string) {
...
export class TextSplitterStream {
  ...
  private _resolver: ((value?: void | PromiseLike<void>) => void) | null;
